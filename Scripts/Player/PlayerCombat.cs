...
namespace PoEClone2D.Player
{
    public class PlayerCombat : MonoBehaviour
    {
        ...
        private void OnAttackPerformed(InputAction.CallbackContext context)
        {
            ...
            // NEW: Check if clicking on item
            if (IsClickingOnItem())
            {
                Debug.Log("PlayerCombat: Clicking on item, ignoring attack");
                return;
            }
        ...
        // NEW METHOD: Check if clicking on item
        private bool IsClickingOnItem()
        {
            ...
            Ray ray = UnityEngine.Camera.main.ScreenPointToRay(mousePos); // FIXED HERE
            RaycastHit2D hit = Physics2D.Raycast(ray.origin, ray.direction);

            if (hit.collider != null)
            {
                // Check if it's an item pickup
                SimpleClickPickup pickup = hit.collider.GetComponent<SimpleClickPickup>();
                if (pickup != null)
                {
                    return true;
                }
