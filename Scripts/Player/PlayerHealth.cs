...
namespace PoEClone2D.Player
{
    public class PlayerHealth : MonoBehaviour, IDamageable
    {
        ...
        public void TakeDamage(float damage)
        {
            ...
            // Visual feedback - IMMEDIATELY flash, don't wait for coroutine
            if (spriteRenderer != null && spriteRenderer.enabled)
            {
                ...
                // Restore after delay
                StartCoroutine(RestoreColorAfterFlash(currentColor));
            }
        ...
        private IEnumerator RestoreColorAfterFlash(Color originalColor)
        {
        ...
        {
            // Don't apply knockback if dead
            if (isDead) return;

            Debug.Log($"Player knockback applied: {force}");

            // Get Rigidbody2D from player controller or this object
            Rigidbody2D rb = GetComponent<Rigidbody2D>();
            if (rb != null)
            {
                // IMPORTANT: Clear existing velocity first
                rb.linearVelocity = Vector2.zero;

                // Add knockback force with Impulse for immediate effect
                rb.AddForce(force, ForceMode2D.Impulse);

                Debug.Log($"Player knockback force applied: {force}, current velocity: {rb.linearVelocity}");

                // Optional: Add a small upward component for better visual
                if (Mathf.Abs(force.x) > 0)
                {
                    rb.AddForce(new Vector2(0, Mathf.Abs(force.x) * 0.2f), ForceMode2D.Impulse);
                }
            }
        }
        ...
        {
            Debug.Log("FlashDamage started");

            if (spriteRenderer != null && spriteRenderer.enabled)
            {
                Debug.Log($"Changing color from {spriteRenderer.color} to {damageFlashColor}");
                spriteRenderer.color = damageFlashColor;
                yield return new WaitForSeconds(flashDuration);
                spriteRenderer.color = originalColor;
                Debug.Log($"Changed color back to {originalColor}");
            }
            else if (spriteRenderer != null)
            {
                Debug.LogWarning("SpriteRenderer is disabled, cannot flash!");
            }
            else
            {
                Debug.LogWarning("No spriteRenderer for flash!");
            }
        }
