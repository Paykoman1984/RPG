...
namespace PoEClone2D.Combat
{
    public class MeleeAttack : MonoBehaviour
    {
        ...
        public void EnableHitboxes()
        {
            if (!isAttacking)
            {
                Debug.LogWarning("Tried to enable hitboxes but attack is not active!");
                return;
            }
        ...
        public void CompleteAttack()
        {
            if (!isAttacking)
            {
                Debug.LogWarning("Tried to complete attack but attack is not active!");
                return;
            }
        ...
        {
            if (!isAttacking) return; // Don't cancel if not attacking
