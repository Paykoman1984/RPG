...
namespace PoEClone2D.Combat
{
    public class Hitbox : MonoBehaviour
    {
        ...
        private void ProcessHit(IDamageable damageable, Vector3 hitPosition)
        {
            ...
            // Play hit sound (ONLY when actually hitting something)
            if (attackData != null && attackData.HitSound != null)
            {
                // Get PlayerCombat from owner to play hit sound
                PlayerCombat playerCombat = owner?.GetComponent<PlayerCombat>();
                if (playerCombat != null)
                {
                    playerCombat.PlayAttackHitSound(hitPosition);
                }
                else
                {
                    ...
                    AudioSource.PlayClipAtPoint(
                        attackData.HitSound,
                        hitPosition,
                        attackData.HitVolume * (attackData.RandomizePitch ?
                        Random.Range(attackData.MinPitch, attackData.MaxPitch) : 1f)
                    );
                }
            }
