...
namespace PoEClone2D.Enemy
{
    public class TestEnemy : MonoBehaviour, IDamageable
    {
        ...
        private void DropItemsOnDeath()
        {
            ...
            if (numDrops <= 0)
            {
                //Debug.Log($"<color=yellow>[TestEnemy]</color> No items to drop (minDrops={minDrops}, maxDrops={maxDrops})");
                return;
            }

            if (itemDropPrefabs == null || itemDropPrefabs.Length == 0)
            {
                //Debug.LogWarning("<color=orange>[TestEnemy]</color> No item drop prefabs assigned!");
                return;
            }

            //Debug.Log($"<color=yellow>[TestEnemy]</color> Attempting to drop {numDrops} item(s)...");

            int successfulDrops = 0;

            for (int i = 0; i < numDrops; i++)
            {
                // Check drop chance for each potential drop
            ...
            {
                //Debug.Log($"<color=green>[TestEnemy]</color> Successfully dropped {successfulDrops} item(s)");
            }
            else
            {
                //Debug.Log($"<color=gray>[TestEnemy]</color> Drop chance failed for all {numDrops} potential drops");
            }
        }
