...
namespace PoEClone2D.Enemy
{
    public class TestEnemy : MonoBehaviour, IDamageable
    {
        ...
        [SerializeField] private float dropChance = 1f; // Changed to 100% for testing
        [SerializeField] private int minDrops = 1;      // Changed for testing
        [SerializeField] private int maxDrops = 1;      // Changed for testing
        ...
        {
            Debug.Log($"<color=yellow>[TestEnemy] DropItemsOnDeath() called</color>");

            if (!canDropItems)
            {
                Debug.Log($"<color=red>[TestEnemy] Item drops disabled!</color>");
                return;
            }

            // Try EnemyLoot component first
            EnemyLoot enemyLoot = GetComponent<EnemyLoot>();
            if (enemyLoot != null)
            {
                Debug.Log($"<color=green>[TestEnemy] Using EnemyLoot system</color>");
                enemyLoot.DropLoot(transform.position);
                return;
            }

            // Fallback to legacy system
            Debug.Log($"<color=yellow>[TestEnemy] No EnemyLoot, using legacy system</color>");
            ...
            if (itemDropPrefabs != null && itemDropPrefabs.Length > 0)
            {
                ...
                for (int i = 0; i < numDrops; i++)
                {
                    ...
                    {
                        successfulDrops++;
                        GameObject randomPrefab = itemDropPrefabs[Random.Range(0, itemDropPrefabs.Length)];
                        if (randomPrefab != null)
                        {
                            Vector2 randomOffset = Random.insideUnitCircle * dropRadius;
                            Vector3 dropPosition = transform.position + new Vector3(randomOffset.x, randomOffset.y, 0);
                            Instantiate(randomPrefab, dropPosition, Quaternion.identity);
                        }
                    }
                }

                Debug.Log($"<color=green>[TestEnemy] Dropped {successfulDrops} item(s)</color>");
            }
            else
            {
                Debug.Log($"<color=red>[TestEnemy] No itemDropPrefabs assigned!</color>");
            }
        }

        private void PlayDropEffect(Vector2 position)
        {
            // You could add particle effects or sounds here
            // Example: Instantiate a small sparkle effect
            //Debug.Log($"<color=gray>[TestEnemy]</color> Item dropped at {position}");
        }
        ...
        {
            isDead = true;
            Debug.Log($"<color=red>[TestEnemy] DIED at position {transform.position}</color>");

            // Drop items before disabling components
            DropItemsOnDeath();
        ...
        private void OnGUI()
        {
            ...
            if (canDropItems)
            {
                debugText += $"\nDrops: {itemDropPrefabs?.Length ?? 0} prefabs";
                debugText += $"\nDrop Chance: {dropChance * 100:F0}%";
            }

            GUI.Label(new Rect(screenPos.x, screenPos.y - 120, 200, 160), debugText, style);
        }
